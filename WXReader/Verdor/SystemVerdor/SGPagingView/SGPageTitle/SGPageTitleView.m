...
{
    [self.btnMArr enumerateObjectsUsingBlock:^(SGPageTitleButton *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [obj setTitleColor:color forState:(UIControlStateNormal)];
        [obj setTitleColor:color forState:(UIControlStateSelected)];
    }];
...
- (void)P_changeSelectedButton:(UIButton *)button {
    ...
    UIFont *defaultTitleFont = kFont15;
    if ([configureTitleSelectedFont.fontName isEqualToString:defaultTitleFont.fontName] && configureTitleSelectedFont.pointSize == defaultTitleFont.pointSize) {
        ...
        if (self.configure.titleTextZoom == YES) {
            [self.btnMArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                UIButton *btn = obj;
                [btn.titleLabel setFont:self.configure.titleFont];
            }];
            
            [button.titleLabel setFont:kBoldFont((1 + self.configure.titleTextZoomRatio) * self.configure.titleFont.pointSize)];
        }
...
//            [originalBtn.titleLabel.layer setContentsScale:originalBtn.transform.a * UIScreen.mainScreen.scale];
//            [originalBtn.titleLabel.layer setNeedsDisplay];
//            [originalBtn.titleLabel.layer displayIfNeeded];
            // targetBtn 缩放
...
//            [targetBtn.titleLabel.layer setContentsScale:targetBtn.transform.a * [UIScreen mainScreen].scale];
//            [targetBtn.titleLabel.layer setNeedsDisplay];
//            [targetBtn.titleLabel.layer displayIfNeeded];
//        }
//    };
}
