...
@interface WXYZ_CoverPageViewController ()
...
@implementation WXYZ_CoverPageViewController
...
- (UIView *)getNeedsView {
    ...
    if (self.isLeft) {
        view = [self.dataSource pageController:self viewForIndex:self.lastIndex direction:PageRollingDirectionLeft];
    } else {
        view = [self.dataSource pageController:self viewForIndex:self.nextIndex direction:PageRollingDirectionRight];
    }
        ...
        if (self.tempController) {
            ...
            if (self.isLeft) { // 左边
                ...
                if (animated) {
                    ...
                    __weak WXYZ_CoverPageViewController *weakSelf = self;
...
- (void)animateSuccess:(BOOL)isSuccess {
    if (self.contentModeController) {
        if (isSuccess) {
            ...
            if (self.isLeft) {
                [self.dataSource pageController:self viewForIndex:self.lastIndex direction:PageRollingDirectionRight];
            } else {
                [self.dataSource pageController:self viewForIndex:self.nextIndex direction:PageRollingDirectionLeft];
            }
        }
        ...
        if ([self.delegate respondsToSelector:@selector(pageController:currentView:currentIndex:direction:)]) {
            [self.delegate pageController:self currentView:self.currentView currentIndex:self.currentIndex direction:self.isLeft?PageRollingDirectionRight:PageRollingDirectionLeft];
        }
    }
}
...
//            return;
        ...
        if (self.contentModeController) {
            ...
            if ([self.dataSource respondsToSelector:@selector(pageController:viewForIndex:direction:)]) {
                self.tempView = [self.dataSource pageController:self viewForIndex:self.currentIndex direction:PageRollingDirectionNone];
                [self initView];
            }
        }
    }
...
- (void)initViewComplented {
    ...
    self.tempView = nil;
    if ([self.delegate respondsToSelector:@selector(pageController:currentView:currentIndex:direction:)]) {
        [self.delegate pageController:self currentView:self.currentView currentIndex:self.currentIndex direction:PageRollingDirectionNone];
    }
}
...
- (void)switchToIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    if (self.contentModeController) {
        ...
        if ([self.dataSource respondsToSelector:@selector(pageController:viewForIndex:direction:)]) {
            self.tempView = [self.dataSource pageController:self viewForIndex:self.currentIndex direction:PageRollingDirectionRight];
            [self initView];
        }
    }
}
